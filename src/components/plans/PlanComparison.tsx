...
const rows: { label: string; get: (p: ChildrenPlan) => string }[] = [
    ...
    <>
      ...
      <div className="mt-12 hidden overflow-x-auto rounded-xl border bg-card shadow-card md:block">
        <table className="w-full min-w-[46rem] border-collapse text-left text-sm">
          <caption className="sr-only">Comparison of Assured Life children plans</caption>
          <thead>
            <tr className="bg-surface-strong">
              <th scope="col" className="w-44 p-5 text-xs tracking-[0.14em] uppercase text-navy-soft">
                Plan
              </th>
              {childrenPlans.map((p) => (
