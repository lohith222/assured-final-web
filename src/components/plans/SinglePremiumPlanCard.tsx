...
import type { SinglePremiumPlan } from "@/lib/single-premium-plans";

export function SinglePremiumPlanCard({ plan }: { plan: SinglePremiumPlan }) {
  return (
    <Link
      to="/plans/single-premium/$plan"
    ...
    >
      ...
      <div className="relative flex flex-1 flex-col p-6">
        <span className="absolute left-0 top-0 h-[3px] w-0 bg-gold transition-all duration-500 group-hover:w-16" />
        <h3 className="text-xl leading-snug font-semibold text-balance">{plan.name}</h3>
        <p className="mt-4 text-sm leading-relaxed text-muted-foreground">{plan.purpose}</p>
        <div className="mt-5 border-l-2 border-gold/70 pl-3">
          <p className="text-[0.68rem] font-bold tracking-[0.16em] text-navy-soft uppercase">
            Key benefit area
          </p>
          <p className="mt-1 text-sm leading-relaxed">{plan.benefitArea}</p>
        </div>
        <span className="mt-6 inline-flex flex-1 items-end gap-2 text-sm font-semibold text-primary">
          Explore Plan{" "}
          <ArrowRight
